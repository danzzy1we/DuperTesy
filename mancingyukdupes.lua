...
return function()
    ...
    main.Name = "Main"; main.Size = UDim2.new(0, 500, 0, 420) -- Ditambah tingginya untuk CD settings
    main.Position = UDim2.new(0.5, -250, 0.5, -210); main.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
    ...
    inputFrame.Size = UDim2.new(1, -20, 0, 150); inputFrame.Position = UDim2.new(0, 10, 0, 45)
    ...
    -- COOLDOWN SETTING
    local cdLabel = Instance.new("TextLabel", inputFrame)
    cdLabel.Size = UDim2.new(0.4, 0, 0, 30); cdLabel.Position = UDim2.new(0.02, 0, 0.4, 0); cdLabel.Text = "Cooldown (Min 1.5s):"; cdLabel.TextColor3 = Color3.new(1,1,1); cdLabel.BackgroundTransparency = 1; cdLabel.TextXAlignment = "Left"

    local inCD = Instance.new("TextBox", inputFrame)
    inCD.Size = UDim2.new(0.54, 0, 0, 30); inCD.Position = UDim2.new(0.42, 0, 0.4, 0)
    inCD.Text = "1.5"; inCD.BackgroundColor3 = Color3.fromRGB(45,45,45); inCD.TextColor3 = Color3.new(0, 255, 255)
    Instance.new("UICorner", inCD).CornerRadius = UDim.new(0, 5)

    local randomCheck = Instance.new("TextButton", inputFrame)
    randomCheck.Size = UDim2.new(0.96, 0, 0, 30); randomCheck.Position = UDim2.new(0.02, 0, 0.7, 0)
    ...
    local function createScroll(pos)
        ...
        s.Size = UDim2.new(0.48, -5, 0, 170); s.Position = pos
    ...
    local leftLog = createScroll(UDim2.new(0, 10, 0, 230))
    local rightLog = createScroll(UDim2.new(0.5, 5, 0, 230))

    -- BUTTONS
    local spawnBtn = Instance.new("TextButton", main)
    spawnBtn.Size = UDim2.new(0.48, -5, 0, 30); spawnBtn.Position = UDim2.new(0, 10, 0, 195)
    ...
    pubBtn.Size = UDim2.new(0.48, -5, 0, 30); pubBtn.Position = UDim2.new(0.5, 5, 0, 195)
    ...
    -- COOLDOWN LOGIC DENGAN AUTO-CORRECT
    local function getCD()
        local val = tonumber(inCD.Text) or 1.5
        if val < 1.5 then 
            inCD.Text = "1.5"
            return 1.5 
        end
    ...
    local function checkCD()
        local currentCD = getCD()
        if _G.AdminCD and tick() - _G.AdminCD < currentCD then return false end
        _G.AdminCD = tick()
        return true
    end
