-- [[ ADMIN INDEPENDENT CONTROL PANEL - GLOBAL CD TOGGLE EDITION ]] --
return function()
    ...
    main.Position = UDim2.new(0.5, -250, 0.5, -220); main.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
    ...
    local globalCDToggle = Instance.new("TextButton", inputFrame)
    globalCDToggle.Size = UDim2.new(0.46, 0, 0, 30); globalCDToggle.Position = UDim2.new(0.52, 0, 0.32, 0)
    globalCDToggle.Text = "Global CD: ON"; globalCDToggle.BackgroundColor3 = Color3.fromRGB(0, 120, 0); globalCDToggle.TextColor3 = Color3.new(1,1,1)
    Instance.new("UICorner", globalCDToggle).CornerRadius = UDim.new(0, 5)

    local isGlobalCD = true
    globalCDToggle.MouseButton1Click:Connect(function()
        isGlobalCD = not isGlobalCD
        globalCDToggle.Text = isGlobalCD and "Global CD: ON" or "Global CD: OFF"
        globalCDToggle.BackgroundColor3 = isGlobalCD and Color3.fromRGB(0, 120, 0) or Color3.fromRGB(120, 0, 0)
    end)
    ...
    -- COOLDOWN LOGIC
    local function getPrivateCD()
        local val = tonumber(inCD.Text) or 1.5
        if val < 1.5 then inCD.Text = "1.5"; return 1.5 end
        return val
    end

    spawnBtn.MouseButton1Click:Connect(function()
        local currentCD = getPrivateCD()
        if _G.AdminCD and tick() - _G.AdminCD < currentCD then return end
        _G.AdminCD = tick()
    ...
    pubBtn.MouseButton1Click:Connect(function()
        ...
        if isGlobalCD then
            local currentCD = getPrivateCD()
            if _G.GlobalAdminCD and tick() - _G.GlobalAdminCD < currentCD then return end
            _G.GlobalAdminCD = tick()
        end
